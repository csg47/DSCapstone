...
def get_pie_chart(entered_site):
    ...
    # If ALL sites are selected, we will use all rows in the dataframe spacex_df to render 
    # and return a pie chart graph to show the total success launches 
    # (i.e., the total count of class column)
    if entered_site == 'ALL':
        
        totalSuccess = spacex_df.groupby('Launch Site')['class'].count()
        fig = px.pie(spacex_df, values=totalSuccess.values, names=totalSuccess.index, title='Total Success Launches by Site')
        return fig
    # If a specific launch site is selected, you need to filter the dataframe spacex_df 
    # first in order to include the only data for the selected site. 
    # Then, render and return a pie chart graph to show the success (class=1) count
    #  and failed (class=0) count for the selected site.
    else:
        ...
        fig = px.pie(filtered_df,
            ...
            title=f'Total Success / Failures for Site: {entered_site}'
        )
